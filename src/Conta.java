...
public abstract class Conta implements ITaxas, Serializable {
    ...
    public void writeConta() {
        try {
            FileOutputStream fileOut = new FileOutputStream("src/contas/" + agencia + "_" + numero + ".ser");
    ...
    public static Conta getConta(int agencia, int numero){
        Conta aux = null;
        File path = new File("src/contas/");
        File[] files = path.listFiles();

        for(File f : files){
            if(f.getName().equals(agencia+"_"+numero+".ser")){
                System.out.println("Achou"+f.getName());
                try {
